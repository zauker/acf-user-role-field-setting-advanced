<?php 	
	/* 
		Plugin Name: ACF User Role Field Setting Advanced
  ...
  new acf_user_role_field_setting_advanced();
	
	class acf_user_role_field_setting_advanced {
      ...
      if (version_compare($acf_version, '5.5.0', '>=')) {
        ...
        add_filter('acf/validate_value', array($this, 'bypass_validation_for_readonly'), 99, 4);
			}
		} // end public function after_setup_theme

    public function bypass_validation_for_readonly(	$valid, $value, $field, $input) {
      $my_field = $this->prepare_field($field);
      if ($my_field['readonly']){
        return true;
      }
			return $valid;
		}
    ...
    private function user_can_edit($field) {
      ...
      if ($field['_clone']){
				$parent = get_field_object($field['_clone']);
			} else {
				$parent = [];
			}
			if (isset($field['user_roles']) || !empty($parent)) {
				$roles = [];
				if (!empty($field['user_roles']) && is_array($field['user_roles'])) {
					$roles = $field['user_roles'];
				}
				if (!empty($parent['user_roles']) && is_array($parent['user_roles'])) {
					$roles = $parent['user_roles'];
				}
